...
Moghunter.musicbook_command_menu = String(
  Moghunter.parameters["Command Menu"] || "true"
);
Moghunter.musicbook_command_name = String(
  Moghunter.parameters["Command Word"] || "Music Book"
);
Moghunter.musicbook_fade_time = Number(
  Moghunter.parameters["List Fade Duration"] || 120
);
Moghunter.musicbook_completion_word = String(
  Moghunter.parameters["Completion Word"] || "Completion"
);
Moghunter.musicbook_meter_x = Number(
  Moghunter.parameters["Meter X-Axis"] || 90
);
...
Moghunter.musicbook_time_number_x = Number(
  Moghunter.parameters["Number X-Axis"] || 350
);
Moghunter.musicbook_time_number_y = Number(
  Moghunter.parameters["Number Y-Axis"] || 16
);
...
Moghunter.musicbook_titles = [
  ["NameAlice", "Our Beloved", "Title Screen Theme"],
  ["Alice", "Who is She?", "North Village Theme"],
  ["058-Slow01", "To go Beyond", "Green Forest Theme"],
  ["AccumulaTown", "Accumula Town", "Old Village Theme"],
  ["QuennsCastle", "Quenn's Castle", "Office Theme"],
  ["Tavern", "Tavern", "Restaurant Theme"],
];
...
// * PluginCommand
//==============================
var _alias_mog_music_book_pluginCommand =
  Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function (command, args) {
  _alias_mog_music_book_pluginCommand.call(this, command, args);
  if (command === "music_book") {
    $gameSystem.music_book();
  }
  return true;
};
...
Game_System.prototype.bgm_duration = function (type) {
  if (!$gameTemp._bgmBuffer || !$gameTemp._bgmBuffer.isReady()) {
    return 0;
  }
...
Game_System.prototype.bgm_pos = function (type) {
  ...
  if (type === 0) {
    return Math.floor($gameTemp._bgmBuffer.seek());
  } else {
    return $gameTemp._bgmBuffer.seek();
  }
};
...
Game_System.prototype.make_music_list = function () {
  ...
  for (var i = 0; i < music_data.length; i++) {
    ...
    var px = 0;
    var py = 0;
    if (music_data[i][0] === $dataSystem.titleBgm.name) {
      e = true;
    }
    if (music_data[i][1]) {
      t = music_data[i][1];
...
Scene_Map.prototype.create = function () {
  ...
  if (!$gameSystem._music_list) {
    $gameSystem.make_music_list();
  }
};
...
// * PageDown
//==============================
var _alias_mog_music_book_wm_cursorPagedown =
  Window_MusicList.prototype.cursorPagedown;
...
var _alias_mog_music_book_wm_cursorPageup =
  Window_MusicList.prototype.cursorPageup;
...
// * processCancel
//==============================
var _alias_mog_music_book_processCancel =
...
Window_MusicComp.prototype.refresh = function () {
  ...
  this.drawText(
    Moghunter.musicbook_completion_word + " " + comp + " % ",
    0,
    0,
    200,
    "left"
...
Window_Help.prototype.setText_rv = function (text) {
  if (this._text !== text) {
    ...
    for (var i = 0; i < words.length; i++) {
      if (this.textWidth(text1 + words[i]) < this.width - 32) {
        text1 += words[i] + " ";
      } else {
        text2 += words[i] + " ";
      }
...
if (String(Moghunter.musicbook_command_menu) === "true") {
  ...
  // * make Command List
  //==============================
  var _alias_mog_music_book_addOriginalCommands =
  ...
  Window_MenuCommand.prototype.addMusicBook = function () {
    this.addCommand(
      String(Moghunter.musicbook_command_name),
      "music_book",
      true
    );
  };
  ...
  //==============================
  var _alias_mog_music_book_reateCommandWindow =
    Scene_Menu.prototype.createCommandWindow;
  Scene_Menu.prototype.createCommandWindow = function () {
    _alias_mog_music_book_reateCommandWindow.call(this);
    this._commandWindow.setHandler(
      "music_book",
      this.commandMusicBook.bind(this)
    );
...
Scene_Music_Book.prototype.create_particles = function () {
  ...
  for (i = 0; i < 30; i++) {
    ...
    this._sprite_particles[i].y = Math.floor(
      Math.random() * Graphics.boxHeight
    );
...
Scene_Music_Book.prototype.refresh_particles = function () {
  ...
  for (i = 0; i < this._sprite_particles.length; i++) {
    this._sprite_particles[i].bitmap = ImageManager.loadmusicbook(
      String(this.music()[4])
    );
    this.reset_particles(i);
    this._sprite_particles[i].x = Math.floor(Math.random() * Graphics.boxWidth);
...
Scene_Music_Book.prototype.reset_particles = function (i) {
  ...
  this._sprite_particles_data[i][0] = Math.floor(
    Math.random() * 2 * this.music()[5]
  );
  this._sprite_particles_data[i][1] = Math.floor(
    Math.random() * 2 * this.music()[6]
  );
  this._sprite_particles_data[i][2] =
    Math.random() * Moghunter.title_particle_a;
  this._sprite_particles[i].opacity = 0;
  this._sprite_particles[i].x = Math.floor(Math.random() * Graphics.boxWidth);
  var pz = Math.random() * 0.5 * 1;
  this._sprite_particles[i].scale = new PIXI.Point(
    0.5 + Number(pz),
    0.5 + Number(pz)
  );
  if (Moghunter.title_particle_sy < 0) {
    this._sprite_particles[i].y =
      Graphics.boxHeight + this._sprite_particles[i].height * 2;
  ...
  if (
    this._sprite_particles_data[i][0] == 0 &&
    this._sprite_particles_data[i][1] == 0
  ) {
...
Scene_Music_Book.prototype.reset_particles_c = function (i) {
  if (!this._sprite_particles_data) {
    return false;
  }
  if (
    this._sprite_particles[i].x < -this._sprite_particles[i].width * 2 ||
    this._sprite_particles[i].x >
      Graphics.boxWidth + this._sprite_particles[i].width * 2
  ) {
    return true;
  }
  if (
...
Scene_Music_Book.prototype.create_number_time = function () {
  ...
  this._number_pos_data = [
    Moghunter.musicbook_time_number_x,
    Moghunter.musicbook_time_number_y,
  ];
...
Scene_Music_Book.prototype.create_position = function () {
  this._position_data = [
    Moghunter.musicbook_meter_x,
    Moghunter.musicbook_meter_y,
    -1,
    Moghunter.musicbook_point_x,
    Moghunter.musicbook_point_y,
    0,
    0,
    0,
  ];
...
Scene_Music_Book.prototype.refresh_position_data = function () {
  ...
  this._sprite_pos_1.setFrame(
    0,
    0,
    this._position_data[2],
    this._position_data[6]
  );
};
...
Scene_Music_Book.prototype.update_meter = function () {
  ...
  var loop_range =
    (this._position_data[2] * $gameSystem.bgm_loopLength()) /
    $gameSystem.bgm_duration(1);
  this._sprite_pos_4.setFrame(
    0,
    this._position_data[6] * 2,
    loop_range,
    this._position_data[6]
  );
};
...
Scene_Music_Book.prototype.set_m_rate = function () {
  return (
    (this._position_data[2] * $gameSystem.bgm_pos(1)) /
    $gameSystem.bgm_duration(1)
  );
};
...
Scene_Music_Book.prototype.create_window_comp = function () {
  ...
  this._fade2 = [
    0,
    0,
    this._fade[2] - this._w_music_comp.height,
...
Scene_Music_Book.prototype.refresh_music = function () {
  ...
  SoundManager.playOk();
  if (this._playing_index === this._w_music_list._index) {
    return;
  }
...
Scene_Music_Book.prototype.need_refresh = function () {
  if (this._w_music_list._ismoved) {
    return true;
  }
...
Scene_Music_Book.prototype.update_window = function () {
  ...
  if (this.need_refresh()) {
    this._fade[0] = this._fade_time;
  }
  if (this._fade[0] === 0) {
    this._w_music_list.y += 5;
    this._w_music_comp.y += 5;
    this._w_music_list.opacity -= 5;
    if (this._w_music_list.y > this._fade[2]) {
      this._w_music_list.y = this._fade[2];
    }
    if (this._w_music_comp.y > this._fade2[2]) {
    ...
    if (this._w_music_list.y < this._fade[3]) {
      this._w_music_list.y = this._fade[3];
    }
    if (this._w_music_comp.y < this._fade2[3]) {
      this._w_music_comp.y = this._fade2[3];
    }
  }
  if (this._cover_fade) {
    this._cover.opacity -= 5;
  } else {
    this._cover.opacity += 5;
  }
  ...
  if (this._cover.scale.x > 1.0) {
    this._cover.scale.x -= 0.01;
  }
  this._cover.scale.y = this._cover.scale.x;
...
Scene_Music_Book.prototype.need_refresh_number = function () {
  if (this._bgmPos_old != $gameSystem.bgm_pos(0)) {
    return true;
  }
  if (this._bgmduration_old != $gameSystem.bgm_duration(0)) {
    return true;
  }
  return false;
};
...
Scene_Music_Book.prototype.update = function () {
  ...
  if (this.need_refresh_number()) {
    this.refresh_number();
    this._number_refresh = 0;
  }
  if (
    Input.isTriggered("ok") ||
    (TouchInput.isTriggered() && this._w_music_list.isTouchedInsideFrame())
  ) {
    this.refresh_music();
  }
  if (this._position_data[2] === -1 && this._sprite_pos_1.bitmap.isReady()) {
    this.refresh_position_data();
  }
  if (this._position_data[2] > -1) {
